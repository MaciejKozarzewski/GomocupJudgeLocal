...
class ai_match(object):
        ...
        if status == 0:
            if "real_time_pos" in self.settings and self.settings["real_time_pos"] == 1:
                for i in range(len(psq)):
                    self.settings["send"](
                        "pos " + psq_to_psq([psq[i]], self.board_size).encode("base64").replace("\n", "").replace("\r",
                                                                                                                  ""))
            ...
            for i in range(len(self.opening), self.board_size ** 2):
                ...
                try:
                    ...
                    if status != -2 and "real_time_pos" in self.settings and self.settings["real_time_pos"] == 1:
                        self.settings["send"](
                            "pos " + psq_to_psq(_psqturn, self.board_size).encode("base64").replace("\n", "").replace(
                                "\r", ""))
                        self.settings["recv"](16)  # received
                    if "real_time_message" in self.settings and self.settings["real_time_message"] == 1:
    ...
    def init_protocol(self,
        ...
        if protocol_type == 'old':
            return old_protocol(cmd, board, timeout_turn, timeout_match, max_memory, game_type, rule, folder,
                                working_dir, tolerance)
        else:
            return new_protocol(cmd, board, timeout_turn, timeout_match, max_memory, game_type, rule, folder,
                                working_dir, tolerance)
